% Run S5C algorithm with hard-coded range of lambda, [2^-1, 2^-2,..., 2^-10].
%
% Param:
...
function [clustering_errors, elapsed_times] = run_S5C(Y,A0,L,numS)
  ...
  for plambda = 1:10
    ...
    elapsed_times(i_algo) = elapsed_time_1 + elapsed_time_2;
    clustering_errors(i_algo) = clustering_error(A,A0);      
    i_algo = i_algo + 1;	
  end    
end
